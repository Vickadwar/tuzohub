  ...
  return (
    <div className="w-full space-y-6 animate-fadeIn pb-12">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
              Manage local districts and towns assigned to regional territories.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <MetricCard label="Total Towns" value={towns.length} sub="Registered urban nodes" />
        <MetricCard label="Mapping Index" value="100%" sub="Assigned to valid regions" />
        <MetricCard label="Logistics Nodes" value="Active" sub="Sub-territory coverage" />
      </div>
            ...
            <Table className="w-full">
              <TableHeader>
                <TableRow className="bg-gray-50/50 dark:bg-white/[0.01]">
                  <TableCell isHeader className="py-3.5 px-6 text-xs font-semibold text-gray-500 dark:text-gray-400">Town name</TableCell>
                  <TableCell isHeader className="py-3.5 px-6 text-xs font-semibold text-gray-500 dark:text-gray-400">Region</TableCell>
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.04]">
                {filteredTowns.length > 0 ? (
                  filteredTowns.map((town) => (
                    <TableRow key={town.id} className="hover:bg-gray-50/50 dark:hover:bg-white/[0.02] transition-colors">
                      <TableCell className="py-3.5 px-6">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-sky-500/10 text-sky-600 dark:text-sky-400 flex items-center justify-center font-bold text-xs shrink-0 border border-sky-500/20 shadow-2xs">
                            {town.name?.charAt(0) || "T"}
                          </div>
                          <span className="text-xs font-bold text-gray-900 dark:text-white">{town.name}</span>
                        </div>
                      </TableCell>
                      <TableCell className="py-3.5 px-6">
                  ...
                  <TableRow>
                    <TableCell colSpan={3} className="py-12 text-center text-xs font-semibold text-gray-400 italic">
                      {searchTerm ? "No towns matching search filter." : "No registered towns found."}
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
