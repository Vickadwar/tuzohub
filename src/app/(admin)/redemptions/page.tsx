...
export default function RedemptionQueue() {
  const { data: stats, isLoading, mutate } = useApi<any>("/loyalty/stats/overview");
  ...
  const handleApprove = async (id: string) => {
    ...
    setProcessingId(id);
    try {
      // For now, we simulate success or call a future payout endpoint
      await authenticatedFetch(`/api/loyalty/redemptions/${id}/approve`, {
        method: "POST"
      });
      mutate();
    } catch (err) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Review and approve mobile money point-to-cash conversions</p>
        </div>
                    ...
                    <TableRow key={req.id} className="hover:bg-gray-50/50 dark:hover:bg-white/[0.02] transition-colors">
                      <TableCell className="py-4 px-6">
                        <div className="flex flex-col">
                            <span className="text-sm font-bold text-gray-900 dark:text-white">{req.consumer?.firstName} {req.consumer?.lastName}</span>
                      ...
                      <TableCell className="py-4 px-6">
                         <div className="flex flex-col">
                            <span className="text-sm font-black text-gray-900 dark:text-white">{parseFloat(req.amountValue).toLocaleString()} KES</span>
                      ...
                      <TableCell className="py-4 px-6 text-right">
                        <div className="flex items-center justify-end gap-2">
                            ...
                            <button className="p-1.5 text-gray-400 hover:text-error-500 transition-colors">
